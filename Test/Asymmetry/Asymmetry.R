...
## rm(list=ls())
##
fun1 <- function(N, a, tol) {
  m <- matrix(0+0i,N,N)
  m[cbind(1L:(N-1L),2L:N)] <- complex(mod=1,arg=a*pi/N)
  m[cbind(2L:N,1L:(N-1L))] <- complex(mod=1,arg=-a*pi/N)
  m[cbind(1L:(N-1L),2L:N)] <- complex(mod=1,arg=a*pi/N)
  m[cbind(2L:N,1L:(N-1L))] <- complex(mod=1,arg=-a*pi/N)
...
##
plot_fun1 <- function(x, i, ...) {
  p <- par(no.readonly=TRUE)
  par(mfrow=c(2L,1L), mar=c(1,5,4,2))
  plot(x$values,type="l",xaxt="n",main=sprintf("U[%d]",i),...)
  points(x=i,y=x$values[i],pch=21L,bg="black")
  par(mar=c(5,5,1,2))
  sign <- ifelse(diff(Re(x$vectors[1L:2L,i]))>0,1,-1) 
  plot(sign*Re(x$vectors[,i]),type="l",ylab="Value",
       ylim=max(abs(range(Re(x$vectors[,i]),Im(x$vectors[,i]))))*c(-1,1),...)
  lines(sign*Im(x$vectors[,i]),col="red",...)
  par(p)
  ## invisible(NULL)
}
##
x <- fun1(50L, 0.25, .Machine$double.eps^0.5)
##
plot_fun1(x, 1L, las=1)
##
a <- 1
while(!is.null(locator(1L))) {
  x <- fun1(20L, a, .Machine$double.eps^0.5)
  plot_fun1(x, 3L, las=1)
  a <- a + 0.1
}
...
for(i in 2:nrow(coords))
  for(j in 1:(i-1)) {
    ...
    if(d <= th) {
      m[i,j] <- complex(
        modulus=1-(d/th)^w,
        argument=mag*cos(a - aa1)/nrow(coords)
      )
      m[j,i] <- complex(
        modulus=1-(d/th)^w,
        argument=mag*cos(a - aa2)/nrow(coords)
      )
    }
  }
## m[1:5,1:5]
...
##
x <- scale(1:N[1L], center=TRUE, scale=FALSE) %>% as.numeric
y <- scale(1:N[2L], center=TRUE, scale=FALSE) %>% as.numeric
rgl::open3d()
##
i <- 1L
tmp <- matrix(emc$vectors[,i],N[1L],N[2L])
rgl::plot3d(NA, zlim=range(Re(tmp),Im(tmp)), xlim=range(x), ylim=range(y),
            xlab="", zlab="", ylab="", box=FALSE, axes=FALSE)
rgl::axes3d(edges = c("x--", "y--", "z-+"))               
##
rgl::surface3d(z=Re(tmp), col=rgb(1,0,0,0.5),
               x=scale(1:N[1L], center=TRUE, scale=FALSE) %>% as.numeric,
               y=scale(1:N[2L], center=TRUE, scale=FALSE) %>% as.numeric)
rgl::surface3d(z=Im(tmp), col=rgb(0,1,0,0.5),
               x=scale(1:N[1L], center=TRUE, scale=FALSE) %>% as.numeric,
               y=scale(1:N[2L], center=TRUE, scale=FALSE) %>% as.numeric)
##
